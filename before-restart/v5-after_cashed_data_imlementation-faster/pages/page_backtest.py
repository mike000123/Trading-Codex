...
def render() -> None:
    ...
    # ── If results already exist, show them immediately regardless of prices ──
    # (st.rerun() after Run Backtest re-renders the page; prices may not be
    #  re-loaded yet on that pass, so we must not gate results behind prices)
    if "bt_result" in st.session_state:
        _render_results()
        # Still show the config form below results so user can run again
        st.divider()
    ...
    if st.button("▶ Run Backtest", type="primary", key="bt_run"):
        ...
        # Force a clean re-render so results are displayed from session_state
        st.rerun()

    # Results are now shown at the top of render() via _render_results()
    # Nothing more to do here — the Run button block above handles everything


def _render_results() -> None:
    """Render backtest results from session_state. Called at top of render()."""
    if "bt_result" not in st.session_state:
        return
