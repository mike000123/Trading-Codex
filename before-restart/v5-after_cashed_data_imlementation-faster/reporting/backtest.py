...
class BacktestEngine:
    ...
    def run(
        ...
        for i in range(1, n):
            ...
            if (open_trade is None and new_direction is not None
                ...
                if self.risk:
                    ...
                    if not check.approved:
                        ...
                        continue
                    effective_sl      = check.adjusted_sl or suggested_sl
                    effective_capital = check.adjusted_size or capital_per_trade
                else:
                    effective_sl      = suggested_sl
                ...
                open_trade = TradeRecord(
                    ...
                    notes             = (
                                        f"Entry: {new_direction.value} @ {entry_px:.4f} | "
                                        f"SL={effective_sl:.4f} | "
                                        + (f'TP={suggested_tp:.4f}' if suggested_tp is not None else 'TP=none')
                                       ),
                )
