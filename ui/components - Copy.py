...
def render_data_source_selector() -> Optional[pd.DataFrame]:
    ...
    if source == "Yahoo Finance":
        ...
        if not settings.alpaca.has_paper_credentials():
            ...
            col1, col2 = st.sidebar.columns(2)
            start = col1.date_input("Start", value=(pd.Timestamp.today() - pd.Timedelta(days=730)).date(), key="alp_start")
            ...
            if st.sidebar.button("🔄 Fetch / Update from Alpaca", key="alp_fetch"):
                ...
                with st.spinner("Checking for new bars…"):
                    try:
                        data = load_from_alpaca_history(symbol, tf, start_ts, end_ts, key_, sec_, paper=not settings.is_live())
                        st.session_state["loaded_data"] = data
                        st.session_state["loaded_symbol"] = symbol.upper()
                        for k in list(st.session_state.keys()):
        ...
        for i, (param, default) in enumerate(grouped_params):
            ...
            with col:
                if isinstance(default, bool):
                    ...
                    filled[param] = st.number_input(label, value=float(default), format="%.2f", min_value=0.0, key=key, help=help_)
                else:
