...
def themed_progress_markup(
    progress: float,
    *,
    theme_name: str | None = None,
    label: str | None = None,
) -> str:
    """Return themed HTML markup for a progress bar."""
    active_name = theme_name or current_theme_name()
    theme = _THEMES.get(active_name, _THEMES["Dark"])
    ...
    text_secondary = theme.get("--text-secondary", "#9e9eb8")
    accent = theme.get("--accent", "#7c83fd")
    accent_hover = theme.get("--accent-hover", accent)
    border = theme.get("--border", "#2a2d3e")
    safe_label = html.escape(label or f"{pct_label}% complete")
...
<div style="margin-top:0.55rem;">
  <div
    style="
      ...
      border-radius:999px;
      overflow:hidden;
      background:linear-gradient(180deg, {bg_secondary} 0%, {bg_card} 100%);
  ...
  <div
    ...
    "
  >
    <span style="color:{text_primary}; font-weight:600;">{pct_label}%</span>
...
"""
    ...
    # Silver post-processing happens AFTER the css is built so it can swap
    # gold values that live in the always-loaded block (checkboxes, radio
    # bullets, primary-button gradient) too — not only the mrmi_extras
    # conditional. Storing the swap pairs here; applied after css = f"""…""".
    _silver_swaps: list[tuple[str, str]] = []
    if theme_name == "MRMI Silver":
        _silver_swaps = [
...
"""
    # Apply the silver swap last so it catches all gold values across the
    # whole CSS payload (including always-loaded rules for checkboxes,
    # radios, buttons, expander headers, dataframe headers, etc.).
    for _old_hex, _new_hex in _silver_swaps:
        css = css.replace(_old_hex, _new_hex)
