...
class BacktestEngine:
    ...
    def run(
        ...
        progress_cb: Optional[Callable[[float, Any, int], None]] = None,
    ) -> BacktestResult:
        ...
        def _emit_phase(progress: float, label: str, closed_count: int = 0) -> None:
            if progress_cb is None:
                return
            progress_cb(float(progress), f"phase::{label}", int(closed_count))

        try:
            _emit_phase(0.04, "Generating bulk signals")
            actions_s, meta_s = self.strategy.generate_signals_bulk(data, symbol)
            _emit_phase(0.14, "Bulk signals ready")
            ...
            _emit_phase(0.04, "Generating signals bar-by-bar")
            actions_s = []
            meta_s = []
            progress_step_fallback = max(100, n // 200) if n > 0 else 1
            next_progress_fallback = progress_step_fallback
            for i in range(n):
                ...
                is_last = i >= (n - 1)
                if progress_cb is not None and (is_last or i >= next_progress_fallback):
                    frac = min(max(i / max(n - 1, 1), 0.0), 1.0)
                    _emit_phase(0.04 + (0.10 * frac), f"Generating signals bar-by-bar ({i + 1:,}/{n:,})")
                    while next_progress_fallback <= i:
                        next_progress_fallback += progress_step_fallback
            _emit_phase(0.14, "Signals ready")

        _emit_phase(0.18, "Computing ATR and price arrays")
        ...
        if self.enforce_ssr and "date" in data.columns:
            try:
                ...
                prior_day_close_arr = np.full(n, np.nan, dtype=float)
        _emit_phase(0.22, "Starting main simulation loop")
        ...
        # Emit progress often enough that long simulations feel alive in the UI
        # without redrawing on every bar.
        progress_step_bars = max(100, n // 240) if n > 0 else 1
        next_progress_bar = progress_step_bars

        def _emit_progress(bar_index: int) -> None:
            nonlocal next_progress_bar
            if progress_cb is None or n <= 1:
                return
            is_last = bar_index >= (n - 1)
            if not is_last and bar_index < next_progress_bar:
                return
            progress = min(max(bar_index / max(n - 1, 1), 0.0), 1.0)
            current_ts = dates[min(bar_index, n - 1)]
            closed_count = len(trades) + (1 if open_trade is not None and open_trade.outcome not in (TradeOutcome.OPEN, TradeOutcome.NO_DATA) else 0)
            progress_cb(progress, current_ts, closed_count)
            while next_progress_bar <= bar_index:
                next_progress_bar += progress_step_bars
        ...
        if open_trade is not None:
            # Align standalone backtests with managed-portfolio semantics:
            # value any still-open position at the final available close so the
            # ending equity reflects the full window's mark-to-market result.
            last = data.iloc[-1]
            final_px = float(last["close"])
            pct = (final_px - open_trade.entry_price) / open_trade.entry_price
            if open_trade.direction == Direction.SHORT:
                pct = -pct
            open_trade.leveraged_return_pct = pct * open_trade.leverage * 100
            open_trade.exit_price = final_px
            ...
            equity += trade_pnl
            open_trade.pnl = trade_pnl
            open_trade.notes = f"{open_trade.notes or ''} | Final backtest close".strip(" |")
            trades.append(open_trade)
            if equity_curve:
