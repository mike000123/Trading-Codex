...
QQQ_CACHE = ROOT / "data_cache" / "alpaca" / "QQQ" / "1Min.csv"
...
def _load_qqq_prices() -> pd.DataFrame:
    prices = pd.read_csv(QQQ_CACHE)
    prices["date"] = pd.to_datetime(prices["date"])
    return prices[(prices["date"] >= WINDOW_START) & (prices["date"] <= WINDOW_END)].reset_index(drop=True)


def _effective_qqq_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.effective_default_params(symbol="QQQ"))


def _generic_bollinger_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.default_params())


def _spy_family_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.effective_default_params(symbol="SPY"))


def _prepare_qqq_data(prices: pd.DataFrame) -> pd.DataFrame:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult], EvalResult, EvalResult]:
    tuned = _effective_qqq_defaults()
    generic = _evaluate(prepared_prices, _generic_bollinger_defaults())
    generic.label = "generic_default"
    spy_family = _evaluate(prepared_prices, _spy_family_defaults())
    spy_family.label = "baseline_spy_family"

    search_space: dict[str, list[Any]] = {
        "trend_bias_fast_ema": [120, 156, 195],
        "trend_bias_slow_ema": [624, 780, 975],
        "trend_bias_lookback_bars": [60, 90, 120],
        "trend_bias_min_retrace_pct": [0.4, 0.5, 0.7],
        "trend_bias_min_momentum_120": [0.4, 0.5, 0.7],
        "trend_bias_min_rsi": [45.0, 47.0, 49.0],
        "trend_bias_max_rsi": [70.0, 72.0, 75.0],
        "trend_bias_trail_pct": [2.6, 3.0, 3.4, 4.0],
        "trend_bias_sl_pct": [0.8, 0.9, 1.1, 1.3],
        "trend_bias_cooldown": [60, 90, 120, 180],
        "trend_bias_no_higher_reentry": [False, True],
    }

    history: list[EvalResult] = [generic, spy_family]
    best = _evaluate(prepared_prices, tuned)
    best.label = "baseline_qqq_default"
    history.append(best)
    ...
    return best, history, generic, spy_family


def main() -> None:
    prices = _load_qqq_prices()
    prepared = _prepare_qqq_data(prices)
    best, history, generic, spy_family = _coordinate_sweep(prepared)

    top = sorted(history, key=lambda r: r.score, reverse=True)[:20]
    baseline = next(r for r in history if r.label == "baseline_qqq_default")
    payload = {
        "symbol": "QQQ",
        ...
        "generic_default": asdict(generic),
        "baseline_spy_family": asdict(spy_family),
        "baseline": asdict(baseline),
        "best_1x_current_engine": asdict(best),
        "top_search": [asdict(r) for r in top],
    }

    (ARTIFACT_DIR / "qqq_optimizer_current_engine_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "qqq_optimizer_current_engine_top_search.csv",
        index=False,
    )

    print("Generic default:")
    print(json.dumps(asdict(generic), indent=2))
    print()
    print("SPY-family baseline:")
    print(json.dumps(asdict(spy_family), indent=2))
    print()
    print("Baseline QQQ preset:")
    print(json.dumps(asdict(baseline), indent=2))
    print()
    print("Best QQQ current-engine candidate:")
