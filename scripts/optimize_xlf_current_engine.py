...
XLF_CACHE = ROOT / "data_cache" / "alpaca" / "XLF" / "1Min.csv"
...
def _score_result(result) -> float:
    ...
    sharpe_bonus = (0.0 if pd.isna(result.sharpe_ratio) else float(result.sharpe_ratio)) * 10.0
...
def _load_xlf_prices() -> pd.DataFrame:
    prices = pd.read_csv(XLF_CACHE)
    prices["date"] = pd.to_datetime(prices["date"])
    return prices[(prices["date"] >= WINDOW_START) & (prices["date"] <= WINDOW_END)].reset_index(drop=True)


def _effective_xlf_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.effective_default_params(symbol="XLF"))


def _uso_family_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.effective_default_params(symbol="USO"))


def _prepare_xlf_data(prices: pd.DataFrame) -> pd.DataFrame:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult], EvalResult]:
    tuned = _effective_xlf_defaults()
    uso_family = _evaluate(prepared_prices, _uso_family_defaults())
    uso_family.label = "baseline_uso_family"

    search_space: dict[str, list[Any]] = {
        "rsi_flush_tp_pct": [2.0, 2.5, 3.0],
        "trend_bias_min_momentum_120": [0.6, 0.9, 1.5],
        "shock_rebound_tp_pct": [2.5, 3.5, 4.5],
    }

    history: list[EvalResult] = [uso_family]
    best = _evaluate(prepared_prices, tuned)
    best.label = "baseline_xlf_default"
    history.append(best)
    ...
    return best, history, uso_family


def main() -> None:
    prices = _load_xlf_prices()
    prepared = _prepare_xlf_data(prices)
    best, history, uso_family = _coordinate_sweep(prepared)

    top = sorted(history, key=lambda r: r.score, reverse=True)[:20]
    baseline = next(r for r in history if r.label == "baseline_xlf_default")
    payload = {
        "symbol": "XLF",
        ...
        "baseline_uso_family": asdict(uso_family),
        "baseline": asdict(baseline),
        "best_1x_current_engine": asdict(best),
        "top_search": [asdict(r) for r in top],
    }

    (ARTIFACT_DIR / "xlf_optimizer_current_engine_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "xlf_optimizer_current_engine_top_search.csv",
        index=False,
    )

    print("USO-family baseline:")
    print(json.dumps(asdict(uso_family), indent=2))
    print()
    print("Baseline XLF preset:")
    print(json.dumps(asdict(baseline), indent=2))
    print()
    print("Best XLF current-engine candidate:")
