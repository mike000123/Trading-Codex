...
SLV_CACHE = ROOT / "data_cache" / "alpaca" / "SLV" / "1Min.csv"
...
def _score_result(result) -> float:
    trades_bonus = min(float(result.total_trades), 120.0) * 0.03
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult], dict[str, EvalResult]]:
    ...
    for family_symbol in (None, "GLD", "SPY", "QQQ", "VXZ", "USO"):
    ...
    search_space: dict[str, list[Any]] = {
        ...
        "trend_bias_min_momentum_120": [0.4, 0.7, 1.0],
        "trend_bias_min_atr_pct": [0.02, 0.03, 0.04],
        "trend_bias_min_rsi": [47.0, 49.0, 51.0],
        "trend_bias_max_rsi": [70.0, 72.0, 74.0],
        "trend_bias_trail_pct": [3.6, 4.0, 4.4, 4.8, 5.2],
...
def main() -> None:
    ...
    payload = {
        ...
        "baseline_gld_family": asdict(baselines["baseline_gld_family"]),
    ...
    (ARTIFACT_DIR / "slv_optimizer_current_engine_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "slv_optimizer_current_engine_top_search.csv",
        index=False,
    )

    print("Best SLV current-engine candidate:")
