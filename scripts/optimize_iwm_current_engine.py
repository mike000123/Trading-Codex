...
IWM_CACHE = ROOT / "data_cache" / "alpaca" / "IWM" / "1Min.csv"
...
def _load_iwm_prices() -> pd.DataFrame:
    prices = pd.read_csv(IWM_CACHE)
    prices["date"] = pd.to_datetime(prices["date"])
    return prices[(prices["date"] >= WINDOW_START) & (prices["date"] <= WINDOW_END)].reset_index(drop=True)


def _effective_iwm_defaults() -> dict[str, Any]:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
    return dict(strategy.effective_default_params(symbol="IWM"))
...
def _prepare_iwm_data(prices: pd.DataFrame) -> pd.DataFrame:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult], EvalResult]:
    ...
    best.label = "baseline_iwm_default"
    history.append(best)
...
def main() -> None:
    prices = _load_iwm_prices()
    prepared = _prepare_iwm_data(prices)
    best, history, generic = _coordinate_sweep(prepared)

    top = sorted(history, key=lambda r: r.score, reverse=True)[:20]
    baseline = next(r for r in history if r.label == "baseline_iwm_default")
    payload = {
        "symbol": "IWM",
    ...
    (ARTIFACT_DIR / "iwm_optimizer_current_engine_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "iwm_optimizer_current_engine_top_search.csv",
        index=False,
    )
    ...
    print()
    print("Baseline IWM preset:")
    print(json.dumps(asdict(baseline), indent=2))
    print()
    print("Best IWM current-engine candidate:")
