...
def _score_result(result) -> float:
    ...
    sharpe_bonus = float(result.sharpe_ratio) * 12.0
    dd_penalty = abs(float(result.max_drawdown_pct)) * 1.8
...
def _evaluate(prepared_prices: pd.DataFrame, overrides: dict[str, Any], leverage: float = 1.0) -> EvalResult:
    cls = get_strategy("bollinger_rsi")
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult]]:
    cls = get_strategy("bollinger_rsi")
    baseline_strategy = cls(params={})
    baseline_params = baseline_strategy.effective_default_params(symbol="GLD")
    tuned = dict(baseline_params)

    search_space: dict[str, list[Any]] = {
        "spike_momentum_max": [4, 5, 6],
        "spike_momo_trail_pct": [2.5, 3.0, 3.5, 4.0],
        "spike_momo_sl_pct": [0.9, 1.0, 1.2, 1.4],
        "shock_rebound_tp_pct": [1.4, 1.8, 2.2],
        "shock_rebound_trail_pct": [0.7, 0.9, 1.1, 1.3],
        "intraday_pullback_rsi_trigger": [78.0, 80.0, 82.0, 85.0],
        ...
        "shock_reversal_trail_pct": [0.5, 0.6, 0.8],
    }

    history: list[EvalResult] = []
    best = _evaluate(prepared_prices, tuned, leverage=1.0)
    best.label = "baseline"
    history.append(best)
    ...
    while improved:
        ...
        for key, values in search_space.items():
            ...
            for value in values:
                ...
                candidate = _evaluate(prepared_prices, candidate_params, leverage=1.0)
...
def _leverage_sweep(prepared_prices: pd.DataFrame, params: dict[str, Any]) -> tuple[EvalResult, list[EvalResult]]:
    leverage_candidates = [1.0, 1.5, 2.0, 3.0, 5.0]
    results: list[EvalResult] = []
    best: EvalResult | None = None
    for lev in leverage_candidates:
        current = _evaluate(prepared_prices, params, leverage=lev)
        current.label = f"leverage_{lev:g}x"
...
def main() -> None:
    ...
    best_1x, search_history = _coordinate_sweep(prepared)
    best_lev, leverage_results = _leverage_sweep(prepared, best_1x.params)

    top_search = sorted(search_history, key=lambda r: r.score, reverse=True)[:15]
    top_leverage = sorted(leverage_results, key=lambda r: r.score, reverse=True)
    ...
    payload = {
        ...
        "risk_cap_note": "Leverage sweep uses max 50% capital loss cap via RiskManager.",
        "best_1x": asdict(best_1x),
        "best_leverage": asdict(best_lev),
        "top_search": [asdict(r) for r in top_search],
        "leverage_results": [asdict(r) for r in top_leverage],
    }

    (ARTIFACT_DIR / "gld_optimizer_results.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    pd.DataFrame([asdict(r) for r in top_search]).to_csv(ARTIFACT_DIR / "gld_optimizer_top_search.csv", index=False)
    pd.DataFrame([asdict(r) for r in top_leverage]).to_csv(ARTIFACT_DIR / "gld_optimizer_leverage.csv", index=False)

    print("Best 1x candidate:")
    print(json.dumps(asdict(best_1x), indent=2))
    print()
    print("Best leverage candidate:")
    print(json.dumps(asdict(best_lev), indent=2))
    print()
