...
def _coordinate_sweep(prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult]]:
    ...
    # Focus only on the active GLD exit stack.
    search_space: dict[str, list[Any]] = {
        "spike_momo_trail_pct": [2.5, 3.0, 3.5],
        "spike_momo_sl_pct": [1.0, 1.2, 1.4],
        "intraday_pullback_sl_pct": [0.7, 0.8, 0.9],
        "intraday_pullback_tp_pct": [1.0, 1.2, 1.4],
        "intraday_pullback_trail_pct": [0.5, 0.6, 0.7],
        "shock_reversal_sl_pct": [0.7, 0.8, 0.9],
        "shock_reversal_tp_pct": [0.8, 1.0, 1.2],
        "shock_reversal_trail_pct": [0.4, 0.5, 0.6],
        "cascade_breakdown_sl_pct": [0.7, 0.8, 0.9],
        "cascade_breakdown_tp_pct": [0.8, 1.0, 1.2],
        "cascade_breakdown_trail_pct": [0.5, 0.6, 0.7],
        "shock_rebound_sl_pct": [0.8, 1.0, 1.2],
        "shock_rebound_tp_pct": [1.6, 1.8, 2.0],
        "shock_rebound_trail_pct": [0.7, 0.9, 1.1],
        "rsi_flush_sl_pct": [0.8, 1.0, 1.2],
        "rsi_flush_tp_pct": [0.5, 0.6, 0.7],
        "event_target_sl_pct": [1.2, 1.5, 1.8],
        "event_target_profit_giveback_frac": [0.40, 0.50, 0.60],
        "event_target_profit_giveback_min_pct": [2.0, 3.0, 5.0],
    }
...
def main() -> None:
    ...
    payload = {
        ...
        "best_exit_architecture_1x": asdict(best),
        "top_search": [asdict(r) for r in top],
    }

    (ARTIFACT_DIR / "gld_optimizer_exit_architecture_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "gld_optimizer_exit_architecture_top_search.csv",
        index=False,
    )
    ...
    print()
    print("Best GLD exit-architecture 1x candidate:")
