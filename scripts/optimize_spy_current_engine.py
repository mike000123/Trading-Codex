...
SPY_CACHE = ROOT / "data_cache" / "alpaca" / "SPY" / "1Min.csv"
...
def _prepare_spy_data(prices: pd.DataFrame) -> pd.DataFrame:
    cls = get_strategy("bollinger_rsi")
    strategy = cls(params={})
...
def _coordinate_sweep(prepared_prices: pd.DataFrame) -> tuple[EvalResult, list[EvalResult], EvalResult]:
    tuned = _effective_spy_defaults()
    generic = _evaluate(prepared_prices, _generic_bollinger_defaults())
    generic.label = "generic_default"

    search_space: dict[str, list[Any]] = {
        "trend_bias_fast_ema": [156, 195],
        "trend_bias_lookback_bars": [90, 120],
        "trend_bias_min_retrace_pct": [0.4, 0.5, 0.6],
        "trend_bias_min_momentum_120": [0.4, 0.5, 0.6],
        "trend_bias_min_rsi": [47.0, 48.0],
        "trend_bias_max_rsi": [70.0, 72.0],
        "trend_bias_trail_pct": [3.0, 3.2, 3.4],
        "trend_bias_sl_pct": [0.8, 0.9, 1.0, 1.1],
        "trend_bias_cooldown": [90, 120, 180],
        "trend_bias_no_higher_reentry": [False, True],
    }

    history: list[EvalResult] = [generic]
    best = _evaluate(prepared_prices, tuned)
    best.label = "baseline_spy_default"
    history.append(best)
    ...
    return best, history, generic


def main() -> None:
    prices = _load_spy_prices()
    prepared = _prepare_spy_data(prices)
    best, history, generic = _coordinate_sweep(prepared)

    top = sorted(history, key=lambda r: r.score, reverse=True)[:20]
    baseline = next(r for r in history if r.label == "baseline_spy_default")
    payload = {
        "symbol": "SPY",
    ...
    (ARTIFACT_DIR / "spy_optimizer_current_engine_results.json").write_text(
        json.dumps(payload, indent=2),
        encoding="utf-8",
    )
    pd.DataFrame([asdict(r) for r in top]).to_csv(
        ARTIFACT_DIR / "spy_optimizer_current_engine_top_search.csv",
        index=False,
    )
    ...
    print()
    print("Baseline SPY preset:")
    print(json.dumps(asdict(baseline), indent=2))
    print()
    print("Best SPY current-engine candidate:")
