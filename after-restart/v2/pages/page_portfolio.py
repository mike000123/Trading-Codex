...
def _build_ticker_table(all_trades: list[dict]) -> pd.DataFrame:
    """
    Build a unified ticker summary:
    one row per (symbol, mode) showing open/closed positions,
    total P&L, and a navigation target.
    """
    ...
    for (sym, mode), data in sorted(agg.items()):
        ...
        rows.append({
            ...
            "_nav_target":   MODE_NAV.get(mode, ""),   # hidden, used for nav
        })
...
def _render_mode_tab(mode: str | None, db: Database) -> None:
    ...
    closed = df[df["outcome"].notna() & ~df["outcome"].isin(["Open"]) &
                df["leveraged_return_pct"].notna()]
    ...
    if not closed.empty and "exit_time" in closed.columns:
        ...
        st.altair_chart(
            equity_curve_chart(eq.rename(columns={"exit_time":"date"}),
                               f"Cumulative P&L – {mode or 'All'}"),
    ...
    with st.expander("📋 Trade Log", expanded=False):
        cols = [c for c in ["symbol","direction","entry_price","exit_price","outcome",
        ...
        st.dataframe(fdf[cols].sort_values("entry_time", ascending=False),
                     use_container_width=True)
...
def render() -> None:
    ...
    if all_trades:
        ...
        st.dataframe(
            display_df.style.applymap(_pnl_style, subset=["Total P&L"]),
            use_container_width=True,
        )

        # Navigation buttons for open positions
        ...
        if not open_ticker_rows.empty:
            ...
            for col, (_, row) in zip(nav_cols, open_ticker_rows.iterrows()):
                ...
                if nav_target and col.button(
                    f"→ {row['Symbol']} ({row['Mode']})",
                    key=f"nav_{row['Symbol']}_{row['Mode']}",
                ):
                    # Set session state so app.py navigates there
                    st.session_state["nav_target"] = nav_target
                    # For forward test, pre-select the symbol tab
