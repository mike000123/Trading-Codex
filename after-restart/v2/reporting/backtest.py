...
class BacktestEngine:
    ...
    def run(
        ...
        for i in range(1, n):
            ...
            # Spike-regime trades (spike_long, post_spike_short) are immune.
            # They need to hold through counter-trend bounces; their own
            # pct trailing stop handles exit.
            _spike_trade = open_trade is not None and any(
                r in (open_trade.notes or '')
                for r in ('regime=spike_long', 'regime=post_spike_short')
            )
            if (self.counter_signal_exit
                    and open_trade is not None
                    and new_direction is not None
                    and not _spike_trade):
