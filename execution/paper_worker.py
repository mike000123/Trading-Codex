...
def _worker_loop(stop_event: threading.Event, tick_seconds: int, initial_delay_s: int = 0) -> None:
    ...
    if initial_delay_s > 0:
        log.info(f"paper_worker: delaying first tick for {initial_delay_s}s after startup warmup")
        for _ in range(int(initial_delay_s)):
...
def start(tick_seconds: int = _DEFAULT_TICK_SECONDS, initial_delay_s: int = 0) -> bool:
    ...
    with _LOCK:
        ...
        t = threading.Thread(
            target=_worker_loop,
            args=(_STOP_EVENT, int(tick_seconds), max(0, int(initial_delay_s))),
