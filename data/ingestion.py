...
def _is_alpaca_crypto_symbol(symbol: str) -> bool:
    ...
    return bool(sym.endswith("-USD") or sym.endswith("/USD"))


def _alpaca_crypto_symbol_id(symbol: str) -> str:
...
def _alpaca_recent_sip_cutoff() -> pd.Timestamp:
    # Keep a small safety buffer so long historical requests can still use SIP
    # for the older bulk while IEX covers only the near-present tail.
    now_ts = pd.Timestamp.utcnow()
    if now_ts.tzinfo is not None:
        now_ts = now_ts.tz_convert("UTC").tz_localize(None)
    else:
        now_ts = now_ts.tz_localize(None)
    return now_ts - pd.Timedelta(days=2)
...
def load_from_ticker(
    ...
    if use_cache:
        ...
        if fetch_start is None:
            ...
            cached = _cache.load(source, ticker, interval, start=start_date, end=end_date)
            log.info(f"yfinance CACHE HIT: {ticker}/{interval} — "
                     f"serving from cache, no download needed")
            return _recent_result_put(request_key, cached.reset_index(drop=True))
    else:
        ...
        if fetch_failed is not None or data is None or data.empty:
            if use_cache:
                ...
                if cached is not None and not cached.empty:
                    sliced = cached.reset_index(drop=True)
                    if not sliced.empty:
...
def load_from_alpaca_history(
    ...
    if use_cache:
        ...
        if fetch_start is None:
            cached = _cache.load(source, symbol, timeframe, start=start, end=end)
            log.info(f"Alpaca CACHE HIT: {symbol}/{timeframe} — "
                     f"serving {len(cached)} bars from cache, no API call needed")
    ...
    if _is_alpaca_crypto_symbol(symbol):
        ...
        req = CryptoBarsRequest(
            symbol_or_symbols=_alpaca_crypto_symbol_id(symbol),
            timeframe=tf,
            ...
            if use_cache:
                ...
                if cached is not None and not cached.empty:
                    ...
                    if not sliced.empty:
                        if fetch_failed is not None:
                            log.warning(
                                f"Alpaca crypto fetch failed for {symbol}/{timeframe} "
                                f"({fetch_failed.__class__.__name__}: {fetch_failed}). "
                                f"Serving {len(sliced)} cached bars instead."
                            )
                        else:
                            log.warning(
                                f"Alpaca crypto returned no new bars for {symbol}/{timeframe}. "
                                f"Returning {len(sliced)} cached bars."
                            )
                        return sliced
            if fetch_failed is not None:
                raise ValueError(
                    f"Alpaca crypto fetch failed for {symbol}/{timeframe} and no cached "
                    f"data is available for {start.date()} → {end.date()}: "
                    f"{fetch_failed.__class__.__name__}: {fetch_failed}"
                )
            raise ValueError(
                f"No Alpaca crypto data returned for {symbol} ({timeframe}) "
                f"{fetch_start.date()} → {fetch_end.date()}."
            )
        ...
        if ts_col is None:
            for col in bars.columns:
                if pd.api.types.is_datetime64_any_dtype(bars[col]):
                    ...
                    break
        if ts_col is None:
            raise ValueError(f"Cannot find timestamp in Alpaca crypto response. Columns: {list(bars.columns)}")
        ...
        log.info(
            f"Alpaca crypto API: {symbol} {timeframe} → {len(new_data)} new bars "
            f"({new_data['date'].iloc[0].date()} → {new_data['date'].iloc[-1].date()})"
        )
        if use_cache:
    ...
    def _stock_req(seg_start: pd.Timestamp, seg_end: pd.Timestamp, feed_name: str):
        return StockBarsRequest(
            symbol_or_symbols=symbol,
            timeframe=tf,
            start=seg_start.to_pydatetime(),
            end=seg_end.to_pydatetime(),
            feed=feed_name,
            adjustment="all",
        )
    ...
    for feed_name in ("sip", "iex"):
        req = _stock_req(fetch_start, fetch_end, feed_name)
        try:
    ...
    if (fetch_failed is not None and bars.empty and "recent SIP data" in str(fetch_failed)
            and fetch_start < fetch_end):
        cutoff = _alpaca_recent_sip_cutoff()
        split_frames: list[pd.DataFrame] = []
        old_exc: Exception | None = None
        recent_exc: Exception | None = None
        if fetch_start < cutoff:
            try:
                response = client.get_stock_bars(_stock_req(fetch_start, min(fetch_end, cutoff), "sip"))
                older = response.df
                if older is not None and not older.empty:
                    split_frames.append(older)
            except Exception as exc:
                old_exc = exc
        if fetch_end > cutoff:
            try:
                response = client.get_stock_bars(_stock_req(max(fetch_start, cutoff), fetch_end, "iex"))
                recent = response.df
                if recent is not None and not recent.empty:
                    split_frames.append(recent)
            except Exception as exc:
                recent_exc = exc
        if split_frames:
            bars = pd.concat(split_frames).sort_index()
            used_feed = "split_sip_iex"
            fetch_failed = None
        else:
            fetch_failed = recent_exc or old_exc or fetch_failed
    ...
    elif used_feed == "split_sip_iex":
        log.warning(
            f"Alpaca split-feed fallback in use for {symbol}/{timeframe}: "
            "older history served from SIP, recent tail served from IEX."
        )
...
def _utc_now_naive() -> pd.Timestamp:
    now_ts = pd.Timestamp.utcnow()
    if now_ts.tzinfo is not None:
        return now_ts.tz_convert("UTC").tz_localize(None)
    return now_ts.tz_localize(None)
...
def _load_cached_range(
    ...
    cached = _cache.load(source, symbol, interval, start=start, end=end)
...
def load_forward_blended_data(
    ...
    yahoo_recent_fetch_allowed = True
    yahoo_start_ts = start_ts
    if yahoo_recent_window is not None:
        retention_cutoff = _utc_now_naive() - yahoo_recent_window
        if end_ts < retention_cutoff:
            yahoo_recent_fetch_allowed = False
        else:
    ...
    if alpaca_tf:
        ...
        if yahoo_recent_window is not None and yahoo_recent_fetch_allowed:
            alpaca_cache_end = max(start_ts, end_ts - yahoo_recent_window) - pd.Timedelta(minutes=1)
        cached = _cache.load("alpaca", symbol, alpaca_tf, start=start_ts, end=alpaca_cache_end)
        if cached is not None and not cached.empty:
            frames.append(cached.copy())

    yahoo = None
    if yahoo_recent_fetch_allowed:
        try:
        ...
        # This intraday request ends outside Yahoo's retention window (for
        # example old 1-minute event studies). Avoid impossible fresh fetches
        # and use only whatever local cache already exists.
        yahoo = _load_cached_range("yfinance", symbol, interval, start_ts, end_ts)
        if yahoo is not None and not yahoo.empty:
            log.info(
                f"Forward Yahoo skipped fresh fetch for {symbol}/{interval}: "
                "requested window is older than Yahoo intraday retention, using cache only"
            )
...
def _load_companion_data(
    ...
    if source == "alpaca":
        ...
        if not key_ or not sec_ or not interval:
            return None
        frame = load_from_alpaca_history(symbol, interval, start, end, key_, sec_, paper=not settings.is_live())
    elif source == "yfinance":
        if not interval:
            return None
        frame = load_from_ticker(symbol, interval, start, end)
    elif source == "forward_blend":
        if not interval:
            return None
        frame = load_forward_blended_data(symbol, interval, start, end)
    else:
        frame = None

    if frame_cache is not None and cache_key is not None and frame is not None and not frame.empty:
        frame_cache[cache_key] = frame.copy()
...
def prepare_strategy_data(
    ...
    if not companion_requests or source not in {"alpaca", "yfinance", "forward_blend"}:
        ...
        for _context_key, companion_symbol, prefix in companion_requests:
            try:
                companion = _load_companion_data(
                    source,
                    companion_symbol,
                    interval,
                    start_ts,
