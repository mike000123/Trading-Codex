        ...
        .encode(
            ...
            color=alt.Color(
                ...
                scale=alt.Scale(domain=["Local sim", "Alpaca paper"],
                                range=[_theme_chart_color("primary"), _theme_chart_color("secondary")]),
...
def _histogram(series: pd.Series, title: str, x_label: str,
    ...
    chart = (
        alt.Chart(df)
        .mark_bar(color=_theme_chart_color("primary"), opacity=0.75)
        .encode(
            x=alt.X(
                "value:Q",
    ...
    if "slippage" in title.lower():
        zero = alt.Chart(pd.DataFrame({"v": [0]})).mark_rule(
            color=_theme_chart_color("secondary"), strokeDash=[4, 4]
        ).encode(x="v:Q")
        chart = chart + zero
    return (chart.configure(background="#0c0d14").configure_view(fill="#181a25", strokeOpacity=0).configure_axis(**_axis_cfg()).configure_title(**_title_cfg()))
