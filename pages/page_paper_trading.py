...
_EARNINGS_AUTO_CFG_KEY = "paper_trading_earnings_auto_v1"
_EARNINGS_SUPPRESS_CFG_KEY = "paper_trading_earnings_suppressed_v1"
...
# Prevent backlog/catch-up bars from opening fresh positions long after the
# signal bar has passed. Exits still process through history, but new entries
# only remain eligible for a short real-time window.
_DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS = 180
...
def _align_paper_add_run_with_earnings_gate() -> None:
    if bool(st.session_state.get("pt_earnings_arm")):
        st.session_state["pt_new_strat"] = _EARNINGS_UNIFIED_STRATEGY_NAME
...
# ── Thread-local state shim ──────────────────────────────────────────────────
# When `paper_worker` ticks the same logic from a daemon thread (so phone-locked
# / closed browser sessions still get bars processed every 60s), `st.session_state`
# is unavailable. The shim lets the worker bind a thread-local dict that mimics
# the four mutable session keys (_RUNS, _CACHE, _SIGNALS, _TRAIL); the same
# helper functions then read/write the right container based on caller thread.
# The Streamlit page is unaffected — its callers always fall through to
...
def _state_dict(key: str, default_factory):
    """Return the state container for `key`.

    Mutable returns share their reference with the underlying store, so caller
    mutations (`append`, `__setitem__`, `pop`) persist without an extra write.
    Worker mode (when `_worker_state_local.state` is bound) routes to a
    thread-local dict; page mode routes to `st.session_state`.
    """
    worker = getattr(_worker_state_local, "state", None)
    if worker is not None:
        if key not in worker:
            worker[key] = default_factory()
        return worker[key]
...
def _state_set(key: str, value) -> None:
    """Replace the state container for `key` (worker-local or session)."""
    worker = getattr(_worker_state_local, "state", None)
    if worker is not None:
        worker[key] = value
    else:
...
def _runs_state() -> dict:
    return _state_dict(_RUNS, dict)


def _signals_state() -> list:
    return _state_dict(_SIGNALS, list)


def _trail_state_dict() -> dict:
    return _state_dict(_TRAIL, dict)
...
def _is_hidden_earnings_candidate(run: dict | None) -> bool:
    cfg = run or {}
    return bool(cfg.get("auto_earnings_managed")) and bool(cfg.get("auto_earnings_hidden", False))


def _visible_runs_map(runs: dict | None) -> dict[str, dict]:
    source = runs or {}
    return {
        str(symbol): run
        for symbol, run in source.items()
        if isinstance(run, dict) and not _is_hidden_earnings_candidate(run)
    }
...
def bind_worker_state(state: dict) -> None:
    """Called by `paper_worker` at the start of each tick to mount its
    own dict in place of `st.session_state` for THIS thread only.
    Pass `None` to unbind. Non-worker threads (Streamlit page) are unaffected.
    """
    if state is None:
        if hasattr(_worker_state_local, "state"):
            del _worker_state_local.state
    else:
        _worker_state_local.state = state
...
def _persist_earnings_auto_config(enabled: bool) -> None:
    ...
    try:
        ...
        log.warning(f"Paper earnings auto config save failed: {exc}")
...
def _default_earnings_run(symbol: str) -> dict:
    ...
    return {
        "symbol": sym,
        "interval": "1m",
        "lookback": _PAPER_1M_CONTEXT_FLOOR_BARS,
        "context_floor_enabled": True,
        "two_month_context_floor": True,
        "capital": 500.0,
        "leverage": 1.0,
        "max_loss": 50,
        "spread_pct": 0.06,
        ...
        "equity_aware_sizing": True,
        "shadow_alpaca": False,
        "max_signal_age_seconds": _DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS,
...
def _gate_signal_to_hold(signal, reason: str) -> Signal:
    meta = dict(signal.metadata or {})
    meta["suppressed_action"] = signal.action.value if hasattr(signal.action, "value") else str(signal.action)
    meta["verdict_reason"] = reason
    meta["earnings_reaction_gate"] = True
    return Signal(
...
def _paper_symbol_has_open_position(symbol: str) -> bool:
    sym = str(symbol or "").upper().strip()
    for mode in ("paper", "alpaca_paper"):
        try:
            rows = _db().get_trades(mode=mode, symbol=sym, limit=50)
        except Exception:
            rows = []
        if any(str(row.get("outcome") or "") == "Open" for row in rows):
            return True
    return False
    ...
    for symbol in today_symbols:
        ...
        if existing:
            if existing.get("auto_earnings_managed"):
                ...
                existing.setdefault("auto_earnings_hidden", True)
            else:
                skipped_existing.append(sym)
            continue
...
def _persist_trail_config() -> None:
    trail = _trail_state_dict() or {}
    payload = {
        "trail": _json_safe(trail),
        "saved_at": datetime.utcnow().isoformat(timespec="seconds"),
    }
    try:
        _db().save_config(_TRAIL_CFG_KEY, payload)
    except Exception as exc:
        log.warning(f"Paper trail config save failed: {exc}")
...
def _restore_runs_config() -> bool:
    ...
    for symbol, cfg in runs.items():
        ...
        clean_cfg.setdefault("auto_earnings_hidden", False)
        restored[sym] = clean_cfg

    if not restored:
        return False

    _state_set(_RUNS, restored)
    return True
...
def _restore_trail_config() -> bool:
    if _trail_state_dict():
        return False
    try:
        payload = _db().load_config(_TRAIL_CFG_KEY) or {}
    except Exception as exc:
        log.warning(f"Paper trail config load failed: {exc}")
        return False
    trail = payload.get("trail") or {}
    if not isinstance(trail, dict) or not trail:
        return False
    restored = {str(k): dict(v) for k, v in trail.items() if isinstance(v, dict)}
    if not restored:
        return False
    _state_set(_TRAIL, restored)
    return True
...
def _to_local_series(series: pd.Series) -> pd.Series:
    ...
    Robust to mixed timezone-aware + timezone-naive inputs. Pandas >=2.x
    raises in array_strptime.check_for_mixed_inputs when a column carries
    a mix of tz-aware ISO strings ("...+03:00") and naive ISO strings.
    Streamlit Cloud (pandas 2.x on Python 3.14) hit this on the Recent
    Signals expander; locally older pandas was permissive. Asking pandas
    to normalise everything to UTC up front avoids the mixed-input check.
    """
    try:
        s = pd.to_datetime(series, errors="coerce", utc=True, format="mixed")
    except (TypeError, ValueError):
        # Older pandas (<2.0) didn't accept format="mixed"; fall back.
        try:
            s = pd.to_datetime(series, errors="coerce", utc=True)
        except (TypeError, ValueError):
            # Last-ditch: parse element-wise so a single bad row doesn't
            # take down the whole column.
            s = pd.to_datetime(
                [pd.Timestamp(v).tz_convert("UTC")
                 if pd.notna(v) and getattr(pd.Timestamp(v), "tzinfo", None) is not None
                 else (pd.Timestamp(v).tz_localize("UTC") if pd.notna(v) else pd.NaT)
                 for v in series],
...
def _bar_timestamp(value) -> Optional[pd.Timestamp]:
    if value is None:
        return None
    try:
        ts = pd.Timestamp(value)
...
def _entry_signal_age_seconds(bar_time) -> Optional[float]:
    """Return how old a candidate entry bar is relative to current UTC time."""
    bar_ts = _bar_timestamp(bar_time)
    if bar_ts is None:
        return None
    try:
    ...
    age_s = (now_ts - bar_ts).total_seconds()
    return max(float(age_s), 0.0)


def _entry_signal_is_stale(run: dict, bar_time) -> tuple[bool, Optional[float], int]:
    max_age_s = int(run.get("max_signal_age_seconds") or _DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS)
    if max_age_s <= 0:
        return False, None, max_age_s
    age_s = _entry_signal_age_seconds(bar_time)
    if age_s is None:
        return False, None, max_age_s
    return age_s > max_age_s, age_s, max_age_s


def _format_max_signal_age(max_age_s) -> str:
    try:
        max_age_i = int(max_age_s or 0)
    except Exception:
        max_age_i = _DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS
    return "Off" if max_age_i <= 0 else f"{max_age_i}s"


def _bar_positions_to_process(dates: pd.Series, last_processed_at) -> list[int]:
    normalized = [_bar_timestamp(v) for v in dates]
    valid_positions = [i for i, ts in enumerate(normalized) if ts is not None]
    if not valid_positions:
        return []
    last_processed = _bar_timestamp(last_processed_at)
    if last_processed is None:
        return [valid_positions[-1]]
    return [i for i, ts in enumerate(normalized) if ts is not None and ts > last_processed]
...
def _shadow_orphan_block_reason(symbol: str) -> Optional[str]:
    ...
    if _open_alpaca_paper_trades(symbol):
        return None
    try:
        router = OrderRouter(risk_manager=RiskManager(get_effective_risk_config()))
...
def _trade_requests_session_exit(trade_row: dict) -> bool:
    notes = trade_row.get("notes") or ""
    return "session_exit=eod" in notes


def _is_session_close_bar(ts) -> bool:
    stamp = pd.Timestamp(ts)
    return stamp.hour > 16 or (stamp.hour == 16 and stamp.minute >= 0)
...
def _close_trade(trade_row: dict, *, exit_price: float, exit_time: datetime,
    ...
    _trail_state_dict().pop(trade_row.get("id"), None)
    _persist_trail_config()
    return ret, pnl
...
def _process_symbol_bar(
    ...
    *,
    ...
    for open_t in _open_paper_trades(symbol):
        trail = _trail_state_dict().get(open_t["id"])
    ...
    for open_t in _open_paper_trades(symbol):
        if not _trade_requests_session_exit(open_t) or not _is_session_close_bar(latest_ts):
            continue
        _close_trade(
            open_t,
            exit_price=float(latest["close"]),
            ...
            slippage_pct=run.get("slippage_pct", 0.0),
            commission=run.get("commission", 0.0),
            notes=f"Session-close exit at {float(latest['close']):.4f}",
        )

    raw_signal = strategy.generate_signal(prepared, symbol)
    signal = raw_signal
    ...
    if signal.action != SignalAction.HOLD and not gate_allowed:
        signal = _gate_signal_to_hold(signal, gate_reason)
    ...
    if run.get("counter_signal_exit", True) and raw_signal.action != SignalAction.HOLD:
        new_dir = Direction.LONG if raw_signal.action == SignalAction.BUY else Direction.SHORT
        for open_t in _open_paper_trades(symbol):
    ...
    is_stale_entry, entry_age_s, max_entry_age_s = _entry_signal_is_stale(run, latest_ts)
    if is_stale_entry:
        bar_iso = pd.Timestamp(_bar_timestamp(latest_ts) or latest_ts).isoformat()
        reason = (
            f"Signal bar is stale ({entry_age_s:.0f}s old > {max_entry_age_s}s max) — "
            "new entry skipped."
        )
        run["_last_signal"]["skipped_reason"] = reason
        log.info(
            f"PAPER ENTRY SKIPPED [{symbol}] stale signal bar {bar_iso} "
            f"(age={entry_age_s:.1f}s, max={max_entry_age_s}s)"
        )
        _persist_runs_config()
        return
    ...
    # ── Fair-value cache freshness gate ─────────────────────────────────────
    # When data/fair_value.py falls back to the newest cached snapshot
    # because the live macro fetch failed (stale_fallback), the fair-value
    # regime, slope, and gap inputs are frozen at backtest time. Block new
    # entries so paper-trading doesn't act on stale macro context. Existing
    # positions are not affected — only new entries.
    cache_mode_val = "live"
    try:
        if "fair_value_cache_mode" in prepared.columns:
            cache_mode_val = str(prepared["fair_value_cache_mode"].iloc[-1])
    except Exception:
        cache_mode_val = "live"
    block_on_stale_fv = bool(run.get("block_on_stale_fair_value", True))
    if block_on_stale_fv and cache_mode_val == "stale_fallback":
        reason = (
            "Fair-value cache is on stale_fallback (live macro fetch failed). "
            "New entries are blocked until the cache refreshes. "
            "Disable 'block_on_stale_fair_value' in the run config to override."
        )
        run["_last_signal"]["skipped_reason"] = reason
        _persist_runs_config()
        return
    ...
    if run.get("auto_earnings_hidden"):
        run["auto_earnings_hidden"] = False
    ...
    if trail_state:
        _trail_state_dict()[trade.id] = trail_state
        if persist_state:
            _persist_trail_config()


def _safe_warn(msg: str) -> None:
    """Emit `st.warning` if we're inside a Streamlit script run; otherwise log.
    Lets `_run_tick` be called from `paper_worker`'s daemon thread without
    crashing on the missing ScriptRunContext.
    """
    if getattr(_worker_state_local, "state", None) is not None:
        log.warning(msg)
        return
...
def _rebuild_recent_signals_for_run(
    ...
    if actions is not None and metas is not None and len(actions) == len(full_frame) and len(metas) == len(full_frame):
        ...
        for idx, (_, row) in enumerate(recent_frame.iterrows(), start=start_idx):
            ...
            action_value = actions[idx].value if hasattr(actions[idx], "value") else str(actions[idx])
            gate_allowed, gate_reason = _earnings_gate_allows_entry(symbol, run, row["date"])
            if action_value != SignalAction.HOLD.value and not gate_allowed:
                metadata["suppressed_action"] = action_value
                metadata["verdict_reason"] = gate_reason
                metadata["earnings_reaction_gate"] = True
                action_value = SignalAction.HOLD.value
...
def startup_preload(status_cb=None) -> dict:
    ...
    if not runs:
        return {
            ...
            "message": "No saved Paper Trading runs to preload.",
        }

    loaded: list[dict] = []
    deferred_hidden: list[str] = []
    errors: list[str] = []
    for symbol, run in runs.items():
        if _is_hidden_earnings_candidate(run):
            deferred_hidden.append(symbol)
            continue
        try:
    ...
    return {
        ...
        "deferred_hidden_candidates": deferred_hidden,
        "errors": errors,
        "auto_earnings": auto_summary,
    }
...
def _run_tick(symbol: str, run: dict) -> None:
    ...
    try:
        ...
        return
    if prices is None or prices.empty:
        _safe_warn(f"⚠️ {symbol}: no data returned.")
        return
    ...
    for pos in positions:
        ...
        if latest_bar_ts is None:
            continue
        raw_slice = prices.loc[raw_dates <= latest_bar_ts]
        atr_val = _latest_atr(raw_slice) if not raw_slice.empty else None
        try:
            _process_symbol_bar(
                symbol,
                run,
            ...
            # CRITICAL: even on a per-bar failure, advance the cursor so
            # the next tick doesn't loop forever on the same broken bar.
            # Surface the error on the run so the UI can display it.
            err_iso = pd.Timestamp(latest_bar_ts).isoformat()
            run["_last_processed_bar"] = err_iso
            errors = run.get("_bar_errors") or []
            errors.append({
                "bar": err_iso,
                "error": f"{bar_exc.__class__.__name__}: {bar_exc}",
                "at": pd.Timestamp.utcnow().isoformat(),
            })
            # Cap error list so a stuck bar doesn't bloat the run config.
            run["_bar_errors"] = errors[-20:]
            _safe_warn(
                f"⚠️ {symbol} bar {err_iso}: handler error "
                f"({bar_exc.__class__.__name__}: {bar_exc}). Cursor advanced."
            )
...
def _equity_curve_chart(closed_trades: list[dict], starting_capital: float,
    ...
    plot_df = filter_chart_window(plot_df, window, date_col="time")
    if not df.empty:
    ...
    return (alt.layer(base_line, area, dots)
            .properties(title=alt.TitleParams(title, **_title_cfg()), height=260)
...
def _paper_price_chart(prices: pd.DataFrame, symbol: str,
    ...
    return (alt.layer(*layers)
            .properties(title=alt.TitleParams(
                f"{symbol} – Paper Trading  ▲ BUY  ▼ SELL  ✕ Exit", **_title_cfg()), height=300)
            .configure(background="#0c0d14").configure_view(fill="#181a25", strokeOpacity=0)
...
def render() -> None:
    ...
    # The worker (execution/paper_worker.py) ticks every 60s independent of
    ...
    try:
        ...
        if _status.get("running"):
            ...
            _dur_s = f" · {_dur:.2f}s" if isinstance(_dur, (int, float)) else ""
            _proc = _hb.get("processed") or []
            _proc_s = f" · processed: {', '.join(_proc)}" if _proc else ""
            ...
            st.caption(
                "⚪ Server-side worker: idle (no Alpaca paper credentials). "
                "Browser-driven ticks only — phone-locked sessions will pause."
            )
        else:
            st.caption("⚠️ Server-side worker: not running. Phone-locked sessions will pause.")
    except Exception:
        pass
    ...
    with st.expander("Add Symbol to Paper Trading", expanded=len(st.session_state[_RUNS]) == 0):
        ...
        with guard_col1:
            new_max_signal_age_s = st.number_input(
                "Max signal age (seconds)",
                min_value=0,
                max_value=3600,
                value=_DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS,
                step=30,
                key="pt_max_signal_age_s",
                help=(
                    "New entries are skipped when the signal bar is older than this many "
                    "seconds at processing time. Set to 0 to disable the guard."
                ),
            )
        with guard_col2:
            _current_pt_strategy_name = str(
                st.session_state.get("pt_new_strat", "Bollinger + RSI (Spike-Aware)")
            )
            _current_pt_strategy_id = str(
                strat_names.get(_current_pt_strategy_name, "bollinger_rsi")
            )
            desired_pt_monday_default = (
                False
                if _current_pt_strategy_id == _EARNINGS_UNIFIED_STRATEGY_ID
                else bool(resolve_execution_default(str(new_symbol).upper(), "monday_open_delay", True))
            )
            _monday_auto_sig = f"{str(new_symbol).upper()}|{_current_pt_strategy_id}"
            if st.session_state.get("pt_monday_open_delay_auto_sig") != _monday_auto_sig:
                previous_auto = st.session_state.get("pt_monday_open_delay_last_auto")
                current_value = st.session_state.get("pt_monday_open_delay")
                if "pt_monday_open_delay" not in st.session_state or current_value == previous_auto:
                    st.session_state["pt_monday_open_delay"] = desired_pt_monday_default
                st.session_state["pt_monday_open_delay_last_auto"] = desired_pt_monday_default
                st.session_state["pt_monday_open_delay_auto_sig"] = _monday_auto_sig
            new_enforce_monday_open_delay = st.checkbox(
                "No Monday trades first 30m",
                key="pt_monday_open_delay",
        ...
        auto_select_strategy_name_for_state(
            ...
            locked_strategy_id=(_EARNINGS_UNIFIED_STRATEGY_ID if bool(st.session_state.get("pt_earnings_arm", False)) else None),
        )
        default_strat_name = st.session_state.get("pt_new_strat") or "Bollinger + RSI (Spike-Aware)"
        ...
        new_strat_name = st.selectbox(
            ...
            index=default_idx,
            key="pt_new_strat",
            disabled=bool(st.session_state.get("pt_earnings_arm", False)),
            format_func=lambda name: strategy_option_label_from_name(name, strategies),
        )
        new_strat_id   = strat_names[new_strat_name]
        render_symbol_strategy_guide(new_symbol, mode_label="Paper Trading")
        render_strategy_selection_help(new_strat_id, symbol=new_symbol)
        desired_pt_earnings_arm_default = bool(new_strat_id == _EARNINGS_UNIFIED_STRATEGY_ID)
        if st.session_state.get("pt_earnings_arm_strategy_sig") != new_strat_id:
            previous_auto = st.session_state.get("pt_earnings_arm_last_auto")
            current_value = st.session_state.get("pt_earnings_arm")
            if "pt_earnings_arm" not in st.session_state or current_value == previous_auto:
                st.session_state["pt_earnings_arm"] = desired_pt_earnings_arm_default
            st.session_state["pt_earnings_arm_last_auto"] = desired_pt_earnings_arm_default
            st.session_state["pt_earnings_arm_strategy_sig"] = new_strat_id
        new_arm_only_earnings = st.checkbox(
            "Arm Only On Earnings Reaction Days",
            key="pt_earnings_arm",
            on_change=_align_paper_add_run_with_earnings_gate,
            help=(
        ...
        if new_strat_id == "bollinger_rsi" and str(new_symbol).upper() == "GLD":
            use_rsi_spike_fade_short = st.checkbox(
                ...
                value=False,
                key="pt_gld_rsi_spike_fade_short",
            ...
            use_fair_gap_fade_short = st.checkbox(
                ...
                value=False,
                key="pt_gld_fair_gap_fade_short",
        ...
        if st.button("Add & Start", type="primary", key="pt_add"):
            ...
            run_cfg = {
                ...
                "max_signal_age_seconds": int(new_max_signal_age_s),
    ...
    if "pt_auto_load_earnings_today" not in st.session_state:
        st.session_state["pt_auto_load_earnings_today"] = bool(auto_cfg.get("enabled", False))
    if (
        "pt_earnings_universe_id" not in st.session_state
        or st.session_state["pt_earnings_universe_id"] not in universe_labels
    ):
        st.session_state["pt_earnings_universe_id"] = universe_cfg.get("universe_id")

    st.markdown("**Earnings Automation**")
    ecol1, ecol2 = st.columns(2)
    auto_earnings_today = ecol1.checkbox(
        "Auto-load ALL Today's Earnings Symbols",
        key="pt_auto_load_earnings_today",
        help=(
    ...
    if auto_earnings_today != bool(auto_cfg.get("enabled", False)):
        ...
        if auto_earnings_today:
            with st.spinner("Syncing today's earnings reaction symbols for Paper Trading…"):
                sync_daily_earnings_autoload()
        st.rerun()
    if auto_earnings_today:
    ...
    if not runs:
        st.info("No active paper trading runs. Add a symbol above, or enable earnings automation above.")
        return
    ...
    if refresh_all or (auto and not skip_auto_tick and not page_just_switched):
        ...
        if page_just_switched:
            if _worker_running:
                _sync_session_from_persisted_state()
                runs = _runs_state() or {}
            st.caption("Opened Paper Trading from cached state. Live refresh resumes on the next rerun.")
        else:
    ...
    visible_runs = _visible_runs_map(runs)
    hidden_candidate_symbols = sorted(
        sym for sym, run in runs.items() if _is_hidden_earnings_candidate(run)
    )
    if not visible_runs:
        st.info("No paper-trading tabs are open yet.")
    ...
    summary_rows = []
    for sym, run in visible_runs.items():
        prices  = st.session_state[_CACHE].get(sym)
        ...
        summary_rows.append({
            ...
            "Max Signal Age": _format_max_signal_age(run.get("max_signal_age_seconds")),
            "Mon 09:30-10:00": "Blocked" if run.get("enforce_monday_open_delay") else "Open",
    ...
    _starting_capital = float(sum(float(r.get("capital", 0) or 0) for r in visible_runs.values()))
    ...
    portfolio_chart_window = st.radio(
        "Portfolio chart focus",
        options=CHART_WINDOW_OPTIONS,
        index=0,
        key="pt_portfolio_chart_window",
        horizontal=True,
        format_func=chart_window_label,
    )
    _earliest_start = None
    for r in visible_runs.values():
        sa = r.get("started_at")
        if sa:
            try:
                started_ts = pd.Timestamp(sa)
                if _earliest_start is None or started_ts < _earliest_start:
                    _earliest_start = started_ts
            except Exception:
                pass
    st.altair_chart(
        _equity_curve_chart(closed_paper, _starting_capital,
                             ...
                             window=portfolio_chart_window),
        width='stretch',
    )
    st.divider()

    # ── Per-symbol tabs ──────────────────────────────────────────────────────
    sym_list = list(visible_runs.keys())
    tabs = st.tabs([f"{'●' if visible_runs[s].get('active') else '○'} {s}" for s in sym_list])

    for tab, symbol in zip(tabs, sym_list):
        with tab:
            run    = visible_runs[symbol]
            ...
            with st.expander("⚙️ Run Settings", expanded=False):
                st.caption(
                    f"Entry freshness guard: `max signal age = {_format_max_signal_age(run.get('max_signal_age_seconds'))}`"
                )
                with st.form(key=f"pt_run_settings_{symbol}"):
                    s_col1, s_col2, s_col3 = st.columns(3)
                    with s_col1:
                        edit_max_signal_age_s = st.number_input(
                            "Max signal age (seconds)",
                            min_value=0,
                            max_value=3600,
                            value=int(run.get("max_signal_age_seconds") or _DEFAULT_MAX_ENTRY_SIGNAL_AGE_SECONDS),
                            step=30,
                            key=f"pt_edit_max_signal_age_{symbol}",
                            help=(
                                "New entries are skipped when the signal bar is older than this many "
                                "seconds at processing time. Set to 0 to disable the guard."
                            ),
                    ...
                    with s_col2:
                        edit_monday_delay = st.checkbox(
                            ...
                            key=f"pt_edit_monday_delay_{symbol}",
                            help="Block all entries from the NYSE open until 10:00 ET on Mondays, regardless of strategy.",
                        )
                    with s_col3:
                        edit_arm_only = st.checkbox(
                            ...
                            key=f"pt_edit_earnings_arm_{symbol}",
                            help="Restrict new entries to known earnings reaction days for this symbol.",
                        )
                    save_run_settings = st.form_submit_button("Save Run Settings", type="secondary")
                if save_run_settings:
                    st.session_state[_RUNS][symbol]["max_signal_age_seconds"] = int(edit_max_signal_age_s)
                    ...
                    if not edit_arm_only:
                        ...
                        st.session_state[_RUNS][symbol]["auto_earnings_hidden"] = False
                    _persist_runs_config()
                    st.rerun()
            ...
            chart_window = st.radio(
                ...
                options=CHART_WINDOW_OPTIONS,
                index=0,
                key=f"pt_chart_window_{symbol}",
                horizontal=True,
                format_func=chart_window_label,
            )
            prices_chart = filter_chart_window(prices_local, chart_window)
            ...
            st.altair_chart(
                _paper_price_chart(prices_chart, symbol,
                                    run_signals,
            ...
            if run["strategy_id"] in ("rsi_threshold", "atr_rsi", "vwap_rsi",
                ...
                st.altair_chart(
                    rsi_chart(prices_chart, rsi_p, buy_lvls, sell_lvls)
                        .properties(title=alt.TitleParams(f"{symbol} – Paper Trading RSI ({rsi_p})", **_title_cfg()))
                        .configure(background="#0c0d14").configure_view(fill="#181a25", strokeOpacity=0)
                        .configure_axis(**_axis_cfg())
                        .configure_title(**_title_cfg()),
                    width='stretch',
