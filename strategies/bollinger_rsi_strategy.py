...
──────────────────────────────────────
Bollinger + RSI with Four-Regime Classification (validated)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

FOUR REGIMES (each independently detected):

  1. NORMAL  — standard Bollinger mean reversion, both longs and shorts

  2. SPIKE   — VIX rising (fast OR gradual multi-day)
     Detection: ATR > 2× ATR_MA  OR  price > 5% above its 3-day rolling low
     Action: NO shorts. Longs on dips only.

  3. POST-SPIKE REVERSION — spike occurred AND price has dropped from peak
     Detection: 3-day high > 1.5× 5-day EMA  AND  price < 3-day high × 0.92
     (Independently detected — does NOT require current spike to be active)
     Action: AGGRESSIVE SHORTS. This is UVXY's most profitable regime.

  4. DRIFT   — ATR too low relative to price to cover trading costs
     Detection: ATR < min_atr_pct% of price
     Action: ALL signals paused.

KEY FIXES vs previous version:
  - Gradual multi-day rises now caught via rolling_min (not just ATR/short ROC)
  - Post-spike detection independent of current spike state (spike can have passed)
  - No slope-based drift filter (breaks on structurally declining assets)
"""
...
class BollingerRSIStrategy(BaseStrategy):
    ...
    description = (
        ...
        "Drift (flat). Validated on Aug 2024 and Apr 2025 spike cycles."
    )
    ...
    def default_params(self) -> dict[str, Any]:
        return {
            ...
            "min_rr_ratio":        1.5,
            "cooldown_bars":       5,
            "min_atr_pct":         0.3,    # skip if ATR < X% of price (low vol)
            # ── Spike regime (suppress shorts) ────────────────────────────
            "spike_atr_mult":      2.0,    # ATR > X × ATR_MA → fast spike
            "rise_lookback":       1170,   # 3 trading days rolling min window
            "rise_pct":            5.0,    # price > X% above rise_lookback min → rising
            # ── Post-spike regime (aggressive shorts) ─────────────────────
            "spike_high_window":   1170,   # 3-day rolling high for spike detection
            "spike_ema_mult":      1.5,    # spike if 3d-high > X × 5d-EMA
            "spike_ema_span":      1950,   # 5d EMA span in bars
            "peak_drop_pct":       8.0,    # price dropped X% from 3d-high → post-spike
            "reversion_tp_pct":    15.0,   # post-spike short TP target
            "reversion_sl_pct":    5.0,    # post-spike short SL
            "reversion_rsi_min":   40,     # don't short post-spike if RSI < this (already oversold)
        }
    ...
    def _compute_regimes_bulk(self, close: pd.Series, data: pd.DataFrame, p: dict):
        """Compute all regime flags vectorised. Returns (in_spike, post_spike, is_drift)."""
        atr_s  = _calc_atr(data, 14)
        atr_ma = atr_s.rolling(20, min_periods=5).mean()

        # Spike: fast (ATR) or gradual (price > X% above N-day rolling low)
        ...
        # Post-spike: independent — 3d-high >> 5d-EMA AND price dropped from that high
        ...
        # Drift: ATR too low to cover costs
    ...
    def generate_signal(self, data: pd.DataFrame, symbol: str) -> Signal:
        ...
        if regime == "post_spike" and rsi_v >= int(p["reversion_rsi_min"]):
            ...
            hi_v = float(upper.iloc[-1])
            lo_p = float(lower.iloc[-2])
            hi_p = float(upper.iloc[-2])
            ...
            elif (hi_c and regime == "normal"   # no shorts during spike
    ...
    def generate_signals_bulk(self, data: pd.DataFrame, symbol: str):
        """Vectorised bulk with validated four-regime logic."""
        p = {**self.default_params(), **self.params}
        ...
        last_was_rev    = False  # prevent re-entering post-spike short every bar
        ...
        for pos in range(n):
            ...
            if not (is_long or is_short or is_rev):
                last_was_rev = False if not bool(post_spike.iloc[pos]) else last_was_rev
                continue
            ...
            if is_rev:
                # Only enter post-spike short once per spike (when regime first activates)
                if last_was_rev:
                    continue
