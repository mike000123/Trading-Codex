...
class BollingerRSIStrategy(BaseStrategy):
    ...
    def default_params(self) -> dict[str, Any]:
        return {
            ...
            "spike_gap_pct":       15.0,   # single-bar % change to declare spike onset (15% = real VIX events)
            ...
            "spike_long_trail_pct": 20.0,  # pct trail from running high (grace: skip bar 0)
            ...
            "psshort_drop_pct":     5.0,   # % drop from rolling high to trigger entry
            "psshort_sl_pct":      20.0,   # hard SL % above entry
            "psshort_trail_pct":   15.0,   # pct trail above running low
            "psshort_max":          1,     # 1 entry per spike episode — cleaner
            "psshort_cooldown":    390,    # bars between post-spike shorts
            "psshort_window":      500,    # bars after spike onset to look for first entry (~2.8 days)
    ...
    def _compute_regimes_bulk(self, close: pd.Series, data: pd.DataFrame, p: dict):
        ...
        spike_gap_onset = bar_chg > gap_pct   # fires on the spike bar itself

        # Spike_active: True for psshort_window bars after any spike onset.
        # Used to gate post-spike SHORT entries.
        window = int(p.get("psshort_window", 3000))
        # Efficient: rolling sum — any onset in trailing `window` bars → active
        spike_active = spike_gap_onset.rolling(window, min_periods=1).sum() > 0

        # ── ATR-based spike (kept for BB mean-reversion spike longs) ─────────
        in_spike_atr = (atr_s > float(p["spike_atr_mult"]) * atr_ma) | spike_gap_onset
        ...
        return in_spike_atr, suppress_shorts, post_spike, is_drift, in_decay, atr_s,                spike_gap_onset, spike_active
    ...
    def generate_signals_bulk(self, data: pd.DataFrame, symbol: str):  # noqa: C901
        ...
        psshort_sl_pct    = float(p.get("psshort_sl_pct",    20.0))
        psshort_trail_pct = float(p.get("psshort_trail_pct", 15.0))
