...
from strategies.components import (
    build_apld_btc_overlay_payloads,
...
class BollingerRSIStrategy(BaseStrategy):
    ...
    def companion_contexts(
        ...
        if symbol_u == "APLD" and bool(params.get("apld_btc_overlay_enabled", False)):
            contexts.append("crypto_benchmark")
    ...
    def default_params(self) -> dict[str, Any]:
        return {
            ...
            "min_rr_ratio": 1.5,
            "normal_max_tp_pct": 25.0,
            "normal_max_sl_pct": 25.0,
            "cooldown_bars": 3,
            "min_atr_pct": 0.3,
            ...
            "apld_btc_overlay_enabled": False,
            ...
            "apld_btc_long_qqq_weak_filter_enabled": False,
    ...
    def validate_params(self) -> list[str]:
        ...
        if float(p["normal_max_tp_pct"]) <= 0:
            errors.append("normal_max_tp_pct must be > 0.")
        if float(p["normal_max_sl_pct"]) <= 0:
            errors.append("normal_max_sl_pct must be > 0.")
        ...
        if int(p["apld_btc_short_entry_offset_min"]) < 1:
            errors.append("apld_btc_short_entry_offset_min must be >= 1.")
        if int(p["apld_btc_long_entry_offset_min"]) < 1:
            errors.append("apld_btc_long_entry_offset_min must be >= 1.")
        if float(p["apld_btc_short_btc_threshold"]) <= 0:
            errors.append("apld_btc_short_btc_threshold must be > 0.")
        if float(p["apld_btc_long_btc_threshold"]) <= 0:
            errors.append("apld_btc_long_btc_threshold must be > 0.")
        if float(p["apld_btc_short_gap_threshold"]) <= 0:
            errors.append("apld_btc_short_gap_threshold must be > 0.")
        if float(p["apld_btc_long_gap_threshold"]) <= 0:
            errors.append("apld_btc_long_gap_threshold must be > 0.")
        if float(p["apld_btc_short_peak_threshold"]) <= 0:
            errors.append("apld_btc_short_peak_threshold must be > 0.")
        if float(p["apld_btc_long_trough_threshold"]) <= 0:
            errors.append("apld_btc_long_trough_threshold must be > 0.")
        if float(p["apld_btc_short_pullback_threshold"]) <= 0:
            errors.append("apld_btc_short_pullback_threshold must be > 0.")
        if float(p["apld_btc_long_rebound_threshold"]) <= 0:
            errors.append("apld_btc_long_rebound_threshold must be > 0.")
        if float(p["apld_btc_short_stop_loss_pct"]) <= 0:
            errors.append("apld_btc_short_stop_loss_pct must be > 0.")
        if float(p["apld_btc_long_stop_loss_pct"]) <= 0:
            errors.append("apld_btc_long_stop_loss_pct must be > 0.")
        if float(p["apld_btc_long_qqq_weak_rebound_min_pct"]) <= 0:
            errors.append("apld_btc_long_qqq_weak_rebound_min_pct must be > 0.")
        ...
        if bool(p.get("shock_rebound_long_enabled", False)) and float(p["shock_rebound_rsi_trigger"]) >= float(p["rsi_oversold"]):
    ...
    def generate_signals_bulk(self, data: pd.DataFrame, symbol: str, include_diagnostics: bool = False):  # noqa: C901
        ...
        normal_max_tp_pct = float(p["normal_max_tp_pct"])
        normal_max_sl_pct = float(p["normal_max_sl_pct"])
        ...
        apld_btc_overlay_enabled = bool(p["apld_btc_overlay_enabled"])
        apld_btc_short_enabled = bool(p["apld_btc_short_enabled"])
        apld_btc_long_enabled = bool(p["apld_btc_long_enabled"])
        apld_btc_short_btc_threshold = float(p["apld_btc_short_btc_threshold"])
        apld_btc_short_gap_threshold = float(p["apld_btc_short_gap_threshold"])
        apld_btc_short_entry_offset_min = int(p["apld_btc_short_entry_offset_min"])
        apld_btc_short_peak_threshold = float(p["apld_btc_short_peak_threshold"])
        apld_btc_short_pullback_threshold = float(p["apld_btc_short_pullback_threshold"])
        apld_btc_short_confirm_close_max_pct = float(p["apld_btc_short_confirm_close_max_pct"])
        apld_btc_short_stop_loss_pct = float(p["apld_btc_short_stop_loss_pct"])
        apld_btc_long_btc_threshold = float(p["apld_btc_long_btc_threshold"])
        apld_btc_long_gap_threshold = float(p["apld_btc_long_gap_threshold"])
        apld_btc_long_entry_offset_min = int(p["apld_btc_long_entry_offset_min"])
        apld_btc_long_trough_threshold = float(p["apld_btc_long_trough_threshold"])
        apld_btc_long_rebound_threshold = float(p["apld_btc_long_rebound_threshold"])
        apld_btc_long_confirm_close_min_pct = float(p["apld_btc_long_confirm_close_min_pct"])
        apld_btc_long_stop_loss_pct = float(p["apld_btc_long_stop_loss_pct"])
        apld_btc_long_qqq_weak_filter_enabled = bool(p["apld_btc_long_qqq_weak_filter_enabled"])
        apld_btc_long_qqq_weak_close_max_pct = float(p["apld_btc_long_qqq_weak_close_max_pct"])
        apld_btc_long_qqq_weak_rebound_min_pct = float(p["apld_btc_long_qqq_weak_rebound_min_pct"])
        ...
        long_tp_pct_ok = ((long_tp_d / close_nonzero) * 100.0 <= normal_max_tp_pct).fillna(False)
        long_sl_pct_ok = ((long_sl_d / close_nonzero) * 100.0 <= normal_max_sl_pct).fillna(False)
        long_rr_ok = (((long_tp_d / long_sl_d) >= min_rr) & long_tp_pct_ok & long_sl_pct_ok).fillna(False)
        ...
        short_tp_pct_ok = ((short_tp_d / close_nonzero) * 100.0 <= normal_max_tp_pct).fillna(False)
        short_sl_pct_ok = ((short_sl_d / close_nonzero) * 100.0 <= normal_max_sl_pct).fillna(False)
        short_rr_ok = (((short_tp_d / short_sl_d) >= min_rr) & short_tp_pct_ok & short_sl_pct_ok).fillna(False)
        ...
        bw_pct_arr = bw_pct.to_numpy(dtype=float)
        atr_ok_arr = atr_ok.fillna(False).to_numpy(dtype=bool)
        is_drift_arr = is_drift.fillna(False).to_numpy(dtype=bool)
        post_spike_arr = post_spike.fillna(False).to_numpy(dtype=bool)
        suppress_shorts_arr = suppress_shorts.fillna(False).to_numpy(dtype=bool)
        in_spike_atr_arr = in_spike_atr.fillna(False).to_numpy(dtype=bool)
        long_bb_arr = long_bb.fillna(False).to_numpy(dtype=bool)
        short_bb_arr = short_bb.fillna(False).to_numpy(dtype=bool)
        spike_long_sig_arr = spike_long_sig.fillna(False).to_numpy(dtype=bool)
        normal_long_sig_arr = normal_long_sig.fillna(False).to_numpy(dtype=bool)
        normal_short_sig_arr = normal_short_sig.fillna(False).to_numpy(dtype=bool)
        rev_sig_arr = rev_sig.fillna(False).to_numpy(dtype=bool)
        long_tp_d_arr = long_tp_d.to_numpy(dtype=float)
        long_sl_d_arr = long_sl_d.to_numpy(dtype=float)
        short_tp_d_arr = short_tp_d.to_numpy(dtype=float)
        short_sl_d_arr = short_sl_d.to_numpy(dtype=float)
        long_tp_pct_ok_arr = long_tp_pct_ok.fillna(False).to_numpy(dtype=bool)
        long_sl_pct_ok_arr = long_sl_pct_ok.fillna(False).to_numpy(dtype=bool)
        short_tp_pct_ok_arr = short_tp_pct_ok.fillna(False).to_numpy(dtype=bool)
        short_sl_pct_ok_arr = short_sl_pct_ok.fillna(False).to_numpy(dtype=bool)
        ...
        apld_overlay_payloads: dict[int, dict[str, Any]] = {}
        if symbol_u == "APLD" and apld_btc_overlay_enabled and (apld_btc_short_enabled or apld_btc_long_enabled):
            apld_overlay_payloads = build_apld_btc_overlay_payloads(
                data,
                short_enabled=apld_btc_short_enabled,
                long_enabled=apld_btc_long_enabled,
                short_btc_threshold=apld_btc_short_btc_threshold,
                short_gap_threshold=apld_btc_short_gap_threshold,
                short_entry_offset_min=apld_btc_short_entry_offset_min,
                short_peak_threshold=apld_btc_short_peak_threshold,
                short_pullback_threshold=apld_btc_short_pullback_threshold,
                short_confirm_close_max_pct=apld_btc_short_confirm_close_max_pct,
                short_stop_loss_pct=apld_btc_short_stop_loss_pct,
                long_btc_threshold=apld_btc_long_btc_threshold,
                long_gap_threshold=apld_btc_long_gap_threshold,
                long_entry_offset_min=apld_btc_long_entry_offset_min,
                long_trough_threshold=apld_btc_long_trough_threshold,
                long_rebound_threshold=apld_btc_long_rebound_threshold,
                long_confirm_close_min_pct=apld_btc_long_confirm_close_min_pct,
                long_stop_loss_pct=apld_btc_long_stop_loss_pct,
                long_qqq_weak_filter_enabled=apld_btc_long_qqq_weak_filter_enabled,
                long_qqq_weak_close_max_pct=apld_btc_long_qqq_weak_close_max_pct,
                long_qqq_weak_rebound_min_pct=apld_btc_long_qqq_weak_rebound_min_pct,
            )

        dollar_strength_ret_30_inv = (
            1.0 / max(dollar_strength_block_ret_30, 1e-9)
            if dollar_strength_block_ret_30 > 0
            else 0.0
        )
        dollar_strength_ret_120_inv = (
            1.0 / max(dollar_strength_block_ret_120, 1e-9)
            if dollar_strength_block_ret_120 > 0
            else 0.0
        )
        rates_weakness_ret_30_abs_inv = (
            1.0 / max(abs(rates_weakness_block_ret_30), 1e-9)
            if rates_weakness_block_ret_30 < 0
            else 0.0
        )
        rates_weakness_ret_120_abs_inv = (
            1.0 / max(abs(rates_weakness_block_ret_120), 1e-9)
            if rates_weakness_block_ret_120 < 0
            else 0.0
        )
        long_rates_weakness_ret_30_abs_inv = (
            1.0 / max(abs(long_rates_weakness_block_ret_30), 1e-9)
            if long_rates_weakness_block_ret_30 < 0
            else 0.0
        )
        long_rates_weakness_ret_120_abs_inv = (
            1.0 / max(abs(long_rates_weakness_block_ret_120), 1e-9)
            if long_rates_weakness_block_ret_120 < 0
            else 0.0
        )
        ...
        for pos in range(n):
            ...
            if pos in apld_overlay_payloads:
                overlay_payload = apld_overlay_payloads[pos]
                overlay_action = SignalAction.BUY if overlay_payload.get("action") == "BUY" else SignalAction.SELL
                overlay_meta = dict(overlay_payload.get("metadata", {}) or {})
                overlay_meta.setdefault("rsi", round(rsi_val, 2))
                if include_diagnostics:
                    overlay_gate_values = {
                        "btc_overnight_pct": overlay_meta.get("btc_overnight_pct"),
                        "apld_gap_pct": overlay_meta.get("apld_gap_pct"),
                        "relative_gap_pct": overlay_meta.get("relative_gap_pct"),
                        "entry_offset_min": overlay_meta.get("entry_offset_min"),
                        "stop_loss_pct": overlay_meta.get("stop_loss_pct"),
                    }
                    overlay_meta.setdefault("verdict", overlay_action.value)
                    overlay_meta.setdefault("gate_values", overlay_gate_values)
                    overlay_meta.setdefault(
                        "gate_summary",
                        ", ".join(f"{k}={v}" for k, v in overlay_gate_values.items() if v is not None),
                    )
                actions[pos] = overlay_action
                metas[pos] = {
                    "suggested_tp": overlay_payload.get("suggested_tp"),
                    "suggested_sl": overlay_payload.get("suggested_sl"),
            ...
            dollar_strength_score = 0.0
            if (
                dollar_strength_block_ret_30 > 0
                and dollar_strength_block_ret_120 > 0
                and not np.isnan(dollar_close_now)
                and not np.isnan(dollar_fast_now)
                and not np.isnan(dollar_slow_now)
                and not np.isnan(dollar_ret_30_now)
                and not np.isnan(dollar_ret_120_now)
                ...
                dollar_strength_score = min(
                    2.0,
                    (dollar_ret_30_now * dollar_strength_ret_30_inv + dollar_ret_120_now * dollar_strength_ret_120_inv)
                    * 0.5,
                )
            rates_weakness_score = 0.0
            if (
                rates_weakness_block_ret_30 < 0
                and rates_weakness_block_ret_120 < 0
                and not np.isnan(rates_close_now)
                and not np.isnan(rates_fast_now)
                and not np.isnan(rates_slow_now)
                and not np.isnan(rates_ret_30_now)
                and not np.isnan(rates_ret_120_now)
                ...
                rates_weakness_score = min(
                    2.0,
                    (
                        abs(rates_ret_30_now) * rates_weakness_ret_30_abs_inv
                        + abs(rates_ret_120_now) * rates_weakness_ret_120_abs_inv
                    )
                    * 0.5,
                )
            long_rates_weakness_score = 0.0
            if (
                long_rates_weakness_block_ret_30 < 0
                and long_rates_weakness_block_ret_120 < 0
                and not np.isnan(long_rates_close_now)
                and not np.isnan(long_rates_fast_now)
                and not np.isnan(long_rates_slow_now)
                and not np.isnan(long_rates_ret_30_now)
                and not np.isnan(long_rates_ret_120_now)
                ...
                long_rates_weakness_score = min(
                    2.0,
                    (
                        abs(long_rates_ret_30_now) * long_rates_weakness_ret_30_abs_inv
                        + abs(long_rates_ret_120_now) * long_rates_weakness_ret_120_abs_inv
                    )
                    * 0.5,
                )
            gold_macro_score = (
            ...
            metal_peer_confirm = (
                not np.isnan(metal_peer_close_now)
                and not np.isnan(metal_peer_fast_now)
                and not np.isnan(metal_peer_slow_now)
                and not np.isnan(metal_peer_ret_30_now)
                and not np.isnan(metal_peer_ret_120_now)
                and metal_peer_close_now >= metal_peer_fast_now
                and metal_peer_fast_now >= metal_peer_slow_now
                and metal_peer_ret_30_now >= gold_peer_confirm_ret_30
                and metal_peer_ret_120_now >= gold_peer_confirm_ret_120
            )
            miners_confirm = (
                not np.isnan(miners_close_now)
                and not np.isnan(miners_fast_now)
                and not np.isnan(miners_slow_now)
                and not np.isnan(miners_ret_30_now)
                and not np.isnan(miners_ret_120_now)
                and miners_close_now >= miners_fast_now
                and miners_fast_now >= miners_slow_now
                and miners_ret_30_now >= gold_miners_confirm_ret_30
                and miners_ret_120_now >= gold_miners_confirm_ret_120
            )
            riskoff_override = (
                not np.isnan(riskoff_close_now)
                and not np.isnan(riskoff_fast_now)
                and not np.isnan(riskoff_slow_now)
                and not np.isnan(riskoff_ret_30_now)
                and not np.isnan(riskoff_ret_120_now)
                and riskoff_close_now >= riskoff_fast_now
                and riskoff_fast_now >= riskoff_slow_now
                and riskoff_ret_30_now >= gold_riskoff_ret_30
                and riskoff_ret_120_now >= gold_riskoff_ret_120
            )
            ...
            if episode_phase == "spike" and not in_spike_lockout and strong_spike_long and spike_long_sig_arr[pos]:
            ...
            if episode_phase == "decay":
                ...
                if bounce_failed and decay_bounce_entries < decay_bounce_max and (pos - last_decay_bounce_bar) >= decay_bounce_cooldown:
                    ...
                    if px > decay_floor and atr_ok_arr[pos]:
                        actions[pos] = SignalAction.SELL
                        metas[pos] = {
                            "suggested_tp": None,
                            "suggested_sl": sl,
            ...
            intraday_pullback_ready = intraday_pullback_short_ready(
                ...
                recent_upper_band_rejection=bool(short_bb_arr[pos] or (pos > 0 and short_bb_arr[pos - 1])),
            ...
            shock_rebound_ready = shock_rebound_long_ready(
                ...
                recent_lower_band_rejection=bool(long_bb_arr[pos] or (pos > 0 and long_bb_arr[pos - 1])),
            ...
            allow_normal_long = normal_long_sig_arr[pos]
            ...
            if persistent_rebound_trap:
                allow_normal_long = allow_normal_long and rsi_val <= persistent_rollover_long_rsi_max
            bw_pct_now = bw_pct_arr[pos] if not np.isnan(bw_pct_arr[pos]) else np.nan
            if px <= low_price_chop_price and float(bw_pct_now) <= low_price_chop_bandwidth_pct:
                allow_normal_long = False
            ...
            if include_diagnostics:
                ...
                if (
                    not np.isnan(long_tp_d_arr[pos])
                    and not np.isnan(long_sl_d_arr[pos])
                    and long_sl_d_arr[pos] > 0
                ):
                    normal_long_rr = round(float(long_tp_d_arr[pos] / max(long_sl_d_arr[pos], 1e-9)), 3)
                if (
                    not np.isnan(short_tp_d_arr[pos])
                    and not np.isnan(short_sl_d_arr[pos])
                    and short_sl_d_arr[pos] > 0
                ):
                    normal_short_rr = round(float(short_tp_d_arr[pos] / max(short_sl_d_arr[pos], 1e-9)), 3)
                gate_values = {
                    ...
                    "band_width_pct": round(float(bw_pct_now), 3) if not np.isnan(bw_pct_now) else None,
                    "atr_pct": round(float((atr_now / max(px, 1e-9)) * 100), 3),
                    "min_atr_pct": round(float(p["min_atr_pct"]), 3),
                    "atr_ok": bool(atr_ok_arr[pos]),
                    "is_drift": bool(is_drift_arr[pos]),
                    "post_spike": bool(post_spike_arr[pos]),
                    "suppress_shorts": bool(suppress_shorts_arr[pos]),
                    "in_spike_atr": bool(in_spike_atr_arr[pos]),
                    "long_bb": bool(long_bb_arr[pos]),
                    "short_bb": bool(short_bb_arr[pos]),
                    "normal_long_sig": bool(normal_long_sig_arr[pos]),
                    "normal_short_sig": bool(normal_short_sig_arr[pos]),
                    "normal_long_allowed": bool(allow_normal_long),
                    "normal_long_rr": normal_long_rr,
                    "normal_short_rr": normal_short_rr,
                    "normal_max_tp_pct": round(normal_max_tp_pct, 3),
                    "normal_max_sl_pct": round(normal_max_sl_pct, 3),
                    "normal_long_tp_pct_ok": bool(long_tp_pct_ok_arr[pos]),
                    "normal_long_sl_pct_ok": bool(long_sl_pct_ok_arr[pos]),
                    "normal_short_tp_pct_ok": bool(short_tp_pct_ok_arr[pos]),
                    "normal_short_sl_pct_ok": bool(short_sl_pct_ok_arr[pos]),
                    "recent_max_rsi": round(float(recent_max_rsi_now), 2),
                    "recent_min_rsi": round(float(recent_min_rsi_now), 2),
                ...
                if not normal_short_sig_arr[pos]:
            ...
            if episode_phase != "decay" and allow_normal_long:
                tp = float(sma_arr[pos])
                sl = float(sl_arr[pos])
                tp_pct = ((tp - px) / max(px, 1e-9)) * 100.0
                sl_pct = ((px - sl) / max(px, 1e-9)) * 100.0
                rr = (tp - px) / max(px - sl, 1e-9)
                if rr >= min_rr and tp_pct <= normal_max_tp_pct and sl_pct <= normal_max_sl_pct:
                    actions[pos] = SignalAction.BUY
                    metas[pos] = {
                        "suggested_tp": tp,
                        "suggested_sl": sl,
                ...
                and not gold_macro_bullish
                and px > low_price_chop_price
                and normal_short_sig_arr[pos]
            ):
                tp = float(sma_arr[pos])
                sl = float(sh_sl_arr[pos])
                tp_pct = ((px - tp) / max(px, 1e-9)) * 100.0
                sl_pct = ((sl - px) / max(px, 1e-9)) * 100.0
                rr = (px - tp) / max(sl - px, 1e-9)
                if rr >= min_rr and tp_pct <= normal_max_tp_pct and sl_pct <= normal_max_sl_pct:
                    actions[pos] = SignalAction.SELL
                    metas[pos] = {
